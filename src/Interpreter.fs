...
type EvalContext (assemblyName: AssemblyName, ?dyntypes: bool, ?assemblyResolver: (AssemblyName -> Assembly), ?sink: Sink)  =
    ...
    let dispatchSlots = ConcurrentDictionary<(ResolvedEntity * string * ResolvedTypes),bool>(HashIdentity.Structural)
        ...
        | (REntity (entityType, isShell) as entityR) when not isShell || v.Name = ".ctor" || isAbstractSlot (formalEnv, entityR, v) -> 
            ...
            if v.Name = ".ctor" || v.Name = ".cctor" then 
                ...
                | _res -> 
                    ...
                    | minfo -> RMethod minfo
        | entityR -> 
            interpMethod(formalEnv, entityR, v.Name, v.ArgTypes, range)
                ...
                | DDeclMember (membDef, body, _isLiveCheck) -> 
                    ...
                    let body2 =
                        if membDef.IsValueDef then
                            ...
                            DExpr.Let((loc, body), DExpr.Sequential(DExpr.ValueSet(Choice2Of2 membDef.Ref, DExpr.Value locRef, None), DExpr.Value locRef, membDef.Range))
                        else
                            body
    ...
    member ctxt.EvalExpr(env, expr: DExpr) : Value = 
        ...
        | DExpr.Sequential(firstExpr, secondExpr, range) -> 
            ctxt.EvalSequential(env, firstExpr, secondExpr, range)
